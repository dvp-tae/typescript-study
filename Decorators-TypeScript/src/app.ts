...
function WithTemplate(template: string, hookId: string) {
  ...
  return function<T extends {new(...args: any[]): {name: string}}>(originalConstructor: T) {  
    return class extends originalConstructor {   // 생성자 함수를 만들어 새 로직을 추가하기 위해 클래스를 만듦
      constructor(..._: any[]) {                 // _ 연산자 => 타입스크립트에게 인자가 들어오는 건 알리지만 필요하지 않음을 표시하는 연산자
        super();
...
function Log4(target: any, name: string | Symbol, position: number) {   // 파라미터에 데코레이터 추가할 때는 3번재 인자로 메소드 내 해당 파라미터 위치 정보 넣어줌
...
class Product {
  ...
  getPriceWithTax(@Log4 tax: number) {   // 매개변수에 추가되는 데코레이터
...
function AutoBind(_: any, _2: string, descriptor: PropertyDescriptor) {    // addEventListener 추가 시 this가 가리키는 것을 원본 함수로 bind하기 위한 데코레이터
  const originalMethod = descriptor.value;
  const adjDescriptor: PropertyDescriptor = {
    configurable: true,
    enumerable: false,
    get() {  // 게터는 value 프로퍼티에 값을 반환하기 전에 실행할 부가 로직이 합쳐진 형태
...
function PositiveNumber(target: any, propName: string) {
  registerValidators[target.constructor.name] = {
    ...registerValidators[target.constructor.name],
    [propName]: [...(registerValidators[target.constructor.name]?.[propName] ?? []), 'positive','limit']
  };
}
